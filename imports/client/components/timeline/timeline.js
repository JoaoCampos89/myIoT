...
Template.timelineComponent.helpers({
 timelineEntries: function(){
    var template = Template.instance();

    return template.data.timelineEntries;
  }


});

Template.timelineComponent.onRendered(function(){
...
function Timeline(cvs) {
    ...
    function InitDots() {
        ...
        tl.find('article:last figure').each(function () {
            ...
            self.lines[1].dots.push([canvas.width - template.$(this).outerWidth() - 20, y + 20]);
    ...
    function init() {
        ...
        template.$(canvas).mousemove(function (e) {
            ...
            if (e.offsetX) {
                ...
                mouse.x = e.pageX - template.$(canvas).offset().left;
                mouse.y = e.pageY - template.$(canvas).offset().top;
            }
        });
...
new Timeline(template.$('#cvs3').get(0)).toggle(true);
